/**
 * Cloudflare Pages Function — proxy for Deutsche Nationalbibliothek SRU API.
 * No API key required; this proxy exists solely to bypass CORS restrictions on
 * the DNB SRU endpoint (services.dnb.de) when called from a browser.
 *
 * Usage: GET /api/dnb-isbn?isbn=9783161484100
 */
export async function onRequestGet({ request }) {
  ...
  if (!upstream.ok) return json({ found: false, error: `DNB ${upstream.status}` }, 502)
  ...
  // DNB uses "Lastname, Firstname" — convert to "Firstname Lastname"
